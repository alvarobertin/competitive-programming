...
void suffixIndexDFS(int n, string ac, int h){
  ...
  if(leaf && h >= tamori && !flag){
    tree[n].suffixIndex = tam - h;
    res = tree[n].suffixIndex;
    flag = true;
...
void buildSuffixTree(){
  ...
  tree[0].suffixLink = -1;
  //the first execution of suffixIndexDFS will produce the suffix index in
  //every leaf of tree and print the tree
  suffixIndexDFS(0, "", 0);
}
